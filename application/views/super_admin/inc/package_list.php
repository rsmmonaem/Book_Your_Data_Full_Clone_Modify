...
<div class="page-wrapper">
        <div class="page-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                ...
                <div class="ms-auto">
                    <div class="btn-group">
                        <a href="<?= base_url() ?>super_admin/add_package" type="button" class="btn btn-primary"><i class="bx bx-plus-circle"></i> Add Page for SEO</a>
                    </div>
                </div>
            </div>
            <!--end breadcrumb-->
    ...
    <!-- Include the iziToast library if not already included -->
    ...
    <script>
        ...
        function loadSeoData() {
            $.ajax({
                url: '<?= base_url('super_admin/package_list_content'); ?>',
                type: 'GET',
                dataType: 'json',
                    ...
                    if (response.status === 'success') {
                        ...
                        $.each(seoData, function (index, item) {
                            var rowHtml = '<tr>' +
                                ...
                                '<td>' + item.email_list_database_price + '</td>' +
                                '<td>' + item.number_of_leads + '</td>' +
                                '<td>' + item.status + '</td>' +
                                '<td><a class="btn btn-info edit-seo" href="<?= base_url('super_admin/edit_package/') ?>' + item.email_list_database_id + '">Edit</a></td>' + // Edit link
                                '<td><button class="btn btn-danger delete-seo" data-id="' + item.email_list_database_id + '">Delete</button></td>' +
                                '</tr>';
        ...
        $(document).on("click", ".delete-seo", function () {
            ...
            $.ajax({
                url: '<?php echo base_url("super_admin/delete_package")?>',
                type: 'POST',
                data: { id: id },
                dataType: 'json',
                success: function (response) {
