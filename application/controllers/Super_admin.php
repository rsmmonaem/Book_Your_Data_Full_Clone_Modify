...
/******* ADD Email *******/
    public function add_email(){
        ...
        $this->load->view('super_admin/add_email',$data);
    }
	...
	public function email_list_content() {
		$data['title'] = "Email List";
		$this->load->view('super_admin/email_list',$data);
	}
	...
	public function update_email() {
		...
		redirect('super_admin/email_list_content');
	}

	public function delete_email() {
		$id = $this->input->post('id'); // Retrieve the job function ID from POST data
		$this->load->model('Add_email_model', 'aem');
		$result = $this->aem->delete_email($id);
		if ($result) {
			$response = array(
				'status' => 'success',
...
/******* Order's *******/
		...
		public function edit_orders($id) {
			$data['title'] = "Update";
			$data['email_list_database_id'] = $id;
			$this->load->model('Manage_orders_model', 'mom');
...
public function list_footer_link(){
	$data['title']= "Footer List ";
	$this->load->view('super_admin/list_footer',$data);
}

public function footer_link_list_content() {
	$this->session_data();
	$this->load->model('Package_model', 'pkg');
	$data['title'] = "List Footer";
	$data['data'] = $this->pkg->footer_link_list_content();
	if ($data) {
		$response = array('status' => 'success', 'data' => $data);
	} else {
		$response = array('status' => 'error', 'message' => 'No package found.');
...
public function add_footer_address(){
	$data['title']= "Add Footer address ";
	$data['address'] = 'address';
	$this->load->view('super_admin/add_footer_link',$data);
}

public function list_footer_address(){
	$data['title']= "Add Footer address ";
	$data['address'] = 'address';
	$this->load->view('super_admin/list_footer',$data);
}

public function footer_address_list_content() {
	$this->session_data();
	$this->load->model('Package_model', 'pkg');
	$data['title'] = "List Footer";
	$data['data'] = $this->pkg->footer_address_list_content();
	if ($data) {
		$response = array('status' => 'success', 'data' => $data);
	} else {
		$response = array('status' => 'error', 'message' => 'No package found.');
...
public function delete_footer() {
	$id = $this->input->post('id'); // Retrieve the job function ID from POST data
	$this->load->model('Package_model', 'pkg');
	$result = $this->pkg->delete_footer($id);
	if ($result) {
		$response = array(
			'status' => 'success',
			'message' => 'Deleted successfully.'
